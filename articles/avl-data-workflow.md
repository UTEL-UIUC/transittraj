...
observations and trips. This seven-step process is summarized below:

1.  **Clip points to buffer**: AVL pings beyond a maximum distance from
    the route are removed.

2.  **Project onto route**: AVL latitude-longitude points are projected
    onto the route alignment and converted to one-dimensional distances.
...
6.  **Remove insufficient trips**: Trips with gaps or a short duration
    are removed.
...
loading the libraries we’ll need:
...
For this example, we’ll work with LA Metro’s Line E light rail in the
eastbound direction. Let’s filter our AVL data to these specifications:
...
below that this window has about 3,300 individual GPS pings across 16
trips and 3.5 hours.
...
We’ll note that, even though we’re using rail AVL data here, bus AVL
will work just as well. In fact, `transittraj` was initially designed to
handle bus data specifically.
...
help with this. We’ll start by filtering our GTFS to just the eastbound
Line E:
...
- Your `shape_id`. As shown above, there is only one shape assigned to
  the eastbound Line E (`shape_id = "804EB_RC_221121"`). Many GTFS
  datasets may have multiple shapes for a single route, corresponding to
  short-turns, alternate alignments, etc. Check out
  [`plot_interactive_gtfs()`](https://obrien-ben.github.io/transittraj/reference/plot_interactive_gtfs.md)
  to explore the shapes in your GTFS.
...
- A coordinate reference system. We recommend projecting all spatial
  ...
  WGS UTM zone for Los Angeles, Zone 11N (with an EPSG number of 32611).
...
with `shape_id = "804EB_RC_221121"` and the coordinate system UTM 11N.
...
![](avl-data-workflow_files/figure-html/unnamed-chunk-8-1.png)

For the most part, our GPS pings follow the route alignment excellently.
But what’s the delay with the points off-route north of downtown? This
pings roughly follow the alignment of Line A and end near LA Metro’s
Division 21 yard. These points most likely correspond to one or more
deadheading vehicle that were logged into a trip but were not actually
in revenue service. `transittraj` includes functions intended to
identify and remove potential deadheads. We’ll begin exploring these in
...
- The buffer distance. This will be in the units of your spatial
  projection. Here we’re using WGS UTM, which is in meters. Thus, all
  distance measurements you see in the vignette will be in meters.

For this example, we’ll continue using UTM Zone 11N, and we’ll only keep
...
#> Difference:  50  obs removed
```

We had 50 observations clipped out of our dataset. We can also take a
look at the new header of our dataset:
...
creates problems, as it becomes difficult for `transittraj` to
...
- Should operator IDs be checked? This will look for overlapping
  distinct operator IDs in each trip.
...
The LACMTA dataset does not have include operator IDs, so we will not
...
Below we use
[`clean_overlapping_subtrips()`](https://obrien-ben.github.io/transittraj/reference/clean_overlapping_subtrips.md)
to identify and remove problematic trips:
...
#> # ℹ 1 more variable: n_subtrips_in_range <int>
```

We can see that this trip, ID `63384142`, was removed because at some
point, there were multiple subtrips (i.e., vehicle IDs) in service at
once. Let’s visualize what that looked like:
...
![](avl-data-workflow_files/figure-html/unnamed-chunk-15-1.png)

It looks like, for some reason, a new vehicle – ID `452` – logged into
this trip ID around 7:37, soon after which, data from the original
trainset dropped out before returning shortly after 8:00. Because it’s
...
criteria simple (though not very robust) and use a maximum allowable
deviation of 80 meters. This means that if any given point is more than
80 meters away from the median of its surrounding points, it will be
tossed.
...
Let’s run
[`clean_jumps()`](https://obrien-ben.github.io/transittraj/reference/clean_jumps.md)
as discussed above. We’ll input our maximum allowable distance
deviation, and set the $`t`$ cutoff used by a Hampel filter to infinity
(this will prevent the Hampel filter from removing any points):
...
Let’s some of the pings removed from trip 63383991. We can plot these
points around the fourth row above to see the jump:
...
![](avl-data-workflow_files/figure-html/unnamed-chunk-19-1.png)
...
![](avl-data-workflow_files/figure-html/unnamed-chunk-23-1.png)
...
For this example, we’ll use a minimum trip distance of 1000 meters, and
a minimum trip duration of 120 seconds. Anything will less data than
this will be filtered out. Additionally, we’ll remove trips with a gap
larger than 1000 meters.
...
All of these were removed because of a large gap in data. Let’s take a
look at one, trip 63383948:
...
![](avl-data-workflow_files/figure-html/unnamed-chunk-27-1.png)
...
if our dataset is now monotonic using
...
![](avl-data-workflow_files/figure-html/unnamed-chunk-32-1.png)

In this trip, the GPS backtracks slightly while the train is supposed to
be stopped. The speeds also needed slight adjustments around the
