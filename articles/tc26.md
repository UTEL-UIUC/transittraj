# Transport Chicago 2026 Poster: Supplement

## Introduction

Thanks for checking out `transittraj`! This article contains all code
that produced the figures and stats on our poster at Transport Chicago
2026. As this is intended to supplement the poster presentation, we will
only minimally discuss our code here. If you have additional questions,
...
The data we used here was shared with is privately by IndyGo via their
Swiftly API endpoint. As such, we unfortunately cannot share the raw AVL
data. We’ve pre-loaded the following IndyGo data into our RStudio
workspace before running this vignette:

- `avl_df`: The TIDES-compliant `vehicle_locations.csv` table of
  historic AVL data. This includes all northbound Red Line (90N) trips
  made on weekdays in September and November 2024.

- `route_geom`: The Red Line’s northbound alignment, as retrieved from
  IndyGo’s GTFS via
  [`transittraj::get_shape_geometry()`](https://obrien-ben.github.io/transittraj/reference/get_shape_geometry.md).

- `stops`: The distance of each northbound Red Line stop along its
  alignment, as retrieved from IndyGo’s GTFS via
  [`transittraj::get_stop_distances()`](https://obrien-ben.github.io/transittraj/reference/get_stop_distances.md).

- `signals` and `signal_boundings`: The distance of each northbound
  signal stopbar and entrance-exit analysis zone, respectively. These
  were identified using OpenStreetMap data and Google Earth satellite
  imagery, then projected onto the route via
  [`transittraj::project_onto_route()`](https://obrien-ben.github.io/transittraj/reference/project_onto_route.md).
...
And we can set the parameters we’ll use for our data cleaning.
Throughout the cleaning process, we’ll reference these names variables,
rather than using numeric values.
...
step3_plot <- ggplot(data = plot_df) +
  ...
  geom_point(aes(x = event_timestamp, y = distance, color = op_veh),
            size = 0.8, alpha = 1) +
  scale_color_manual(name = "Operator, Vehicle ID",
  ...
  labs(x = "Time (morning of 9/12/24)",
       ...
       title = "Overlapping Subtrips",
       subtitle = paste("IndyGo Red Line 90 NB; Trip ID ", plot_trip,
                        sep = ""))
step3_plot
```

![](figures/step3_plot_lowres.png)
...
lab_df <- data.frame(y = 14290,
                     ...
                     lab = "*At AVL Ping ID 2012804*<br>**Window Median**: 14158 m<br>**Median Absolute Deviation**: 2.7 m<br>**Distance from Median**: 55 m")
step4_plot <- ggplot(data = plot_df) +
  ...
  geom_textbox(data = lab_df,
             aes(x = x, y = y, label = lab),
             color = "firebrick", size = 3,
             hjust = 0.5, halign = 0.5, width = 0.55) +
  theme_minimal() +
  labs(x = "Time (evening of 9/10/24)",
       y = "Distance from Beginning (m)",
       title = "Outlying Jump in AVL",
       subtitle = paste("IndyGo Red Line 90 NB; Trip ID ", plot_trip,
                        sep = ""))
step4_plot
```

![](figures/step4_plot_lowres.png)
...
step5_plot <- ggplot(data = plot_df) +
  ...
  geom_label(data = (stops %>% filter((distance >= 7000) & (distance <= 11000))),
             ...
             hjust = "left", nudge_y = 0, size = 2.5, color = "grey40", alpha = 0.8) +
  geom_label(data = lab_df,
             aes(x = x, y = y, label = lab),
             color = "firebrick", hjust = "left", size = 3.5) +
  scale_linetype_manual(name = "Features",
                        values = c("BRT Stops" = "dashed")) +
  theme_minimal() +
  labs(x = "Time (afternoon of 9/17/24)",
       y = "Distance from Beginning (m)",
       title = "Gap in AVL Data",
       subtitle = paste("IndyGo Red Line 90 NB; Trip ID ", plot_trip,
                        sep = ""))
step5_plot
```

![](figures/step5_plot_lowres.png)
...
plot_df <- step5_df %>%
  ...
  mutate(point_ok = if_else(condition = (location_ping_id %in% trimmed$location_ping_id),
                            true = "Trim",
                            false = "Ok"))
lab_df <- data.frame(lab = "Minimum distance:\n0 m",
                     x = as.POSIXct("2024-09-16 10:54:37", tz = "America/New_York"),
                     y = 750)
step6_plot <- ggplot(data = plot_df) +
  ...
  geom_label(data = (stops %>% filter((distance <= 7250))),
             ...
             hjust = "right", size = 2, color = "grey40", alpha = 0.8) +
  geom_label(data = lab_df,
             aes(x = x, y = y, label = lab),
             hjust = "center", size = 3.5, color = "firebrick") +
  scale_linetype_manual(name = "Features",
                        values = c("BRT Stops" = "dashed")) +
  theme_minimal() +
  labs(title = "Trimmed Deadhead Along Route",
       subtitle = paste("IndyGo Red Line 90 NB; Trip ID ", plot_trip,
                        sep = ""),
...
step7_plot <- ggplot(data = plot_df1) +
  ...
  labs(x = "Time (evening of 9/10/24)",
       y = "Distance from Beginning (m)",
       title = "Monotonic Correction in AVL",
       subtitle = paste("IndyGo Red Line 90 NB; Trip ID ", plot_trip,
                        sep = ""))
step7_plot
```

![](figures/step7_plot_lowres.png)
...
point_df <- distance_df %>%
  ...
  mutate(speed_lab = paste(round(speed, 1), " m/s", sep = ""))
...
traj_plot <- ggplot() +
  ...
  geom_label(data = point_df,
             ...
             color = "#2f6ff8", size = 3.4, alpha = 0.6, nudge_y = 30, nudge_x = -10) +
  geom_label(data = (stops %>% filter((distance >= plot_lims[1]) & (distance <= plot_lims[2]))),
             aes(x = as.POSIXct("2024-09-25 11:55:50", tz = "America/New_York"),
                 y = distance, label = stop_name),
             color = "firebrick", size = 3.4, hjust = "left", alpha = 0.8) +
  geom_label(data = (signals %>% filter((distance >= plot_lims[1]) & (distance <= plot_lims[2]))),
             aes(x = as.POSIXct("2024-09-25 11:58:50", tz = "America/New_York"),
                 y = distance, label = name),
             color = "grey50", size = 3.4, hjust = "right", nudge_y = -0, alpha = 0.8) +
  scale_linetype_manual(name = "Features",
...
#> 2 1 & 2 1338872    3173 273.747211 -287709 -17.6879602          -4 -0.12590494
#> 3     3 1332860    3158   2.692640   -6012  -0.4490347         -15 -0.47273873
#> 4     4 1330548    3158  62.211240   -2312  -0.1734616           0  0.00000000
#> 5     5 1248044    2897   0.933522  -82504  -6.2007534        -261 -8.26472451
#> 6     6 1241429    2896   0.362689   -6615  -0.5300294          -1 -0.03451847
#> 7     7 1241429    2896 190.054604       0   0.0000000           0  0.00000000
#> 8 Final      NA    2896  34.944517      NA          NA           0  0.00000000
```
...
We’ll use the `transittraj`
[`predict()`](https://rdrr.io/r/stats/predict.html) method with the
`new_distances` parameter set to the entrance & exit position of the
study signal.
...
t_f <- Sys.time()
print(t_f - t_0)
#> Time difference of 1.173125 secs
```
...
lab_df <- plot_vals %>%
  ...
  mutate(delay = if_else(condition = (delay < 1),
                         true = 0,
                         false = delay),
         lab = paste("*", trip_id_performed, "*<br>",
...
delay_plot <- ggplot() +
  ...
  geom_textbox(data = lab_df,
             aes(x = (t_max - 1), y = (y_max - 10), color = traj_type, label = lab),
             size = 2.5, hjust = 1, halign = 1, width = 0.40, alpha = 0.9,
             show.legend = FALSE) +
  ...
  labs(x = "Time from Intersection Entrance (s)",
       y = "Distance from Beginning (m)",
       title = paste("Signal Delay at ", plot_sig, sep = ""),
       subtitle = paste("IndyGo Red Line 90 NB; Free-Flow: ", lab_df$travel_time[1], " s",
                        sep = ""))
delay_plot
```

![](figures/delay_plot_lowres.png)
...
lab_df <- plot_df %>%
  ...
  mutate(delta_mean = round(((mean_delay[2] - mean_delay[1]) / mean_delay[1]) * 100, 1),
         ...
         lab = if_else(condition = (period == "Before (9/2024)"),
                       true = paste("**Mean Delay**: ", mean_delay, " s<br>**85% Delay**: ", delay_85th, " s", "<br>n = ", n_trips, " trips",
                                    sep = ""),
                       false = paste("**Mean Delay**: ", mean_delay, " s (", delta_mean, "%)<br>**85% Delay**: ", delay_85th, " s (", delta_85th, "%)", "<br>n = ", n_trips, " trips",
                                     sep = "")),
         y = c(140, 100))
...
delay_violin <- ggplot() +
  ...
  geom_textbox(data = lab_df,
               ...
               alpha = 0.8, size = 2.7, halign = 0.5, hjust = 0.5, width = 0.53,
               show.legend = FALSE) +
  ...
  labs(x = "Period",
       y = "Signal Delay (s)",
       title = "Distribution of Signal Delay at\nVirginia & South/East",
       subtitle = "IndyGo Red Line 90 NB") +
  ylim(c(0, 150))
delay_violin
```

![](figures/delay_violin_lowres.png)

### Arrival-on-Green & Split Failures

For both of these metric’s, we’ll want to know how many stops each trip
made. We will use the
[`predict()`](https://rdrr.io/r/stats/predict.html) method with the
`distance_lims` and `timestep` inputs to interpolate points for each
trip at a fine resolution, then use run-length-encoding to identify
continuous stretches the vehicle spends stopped (or in motion). We will
consider any speed below 2.24 m/s (5 mph) to be a stop.

``` r

timestep = 0.1 # seconds

# Predict 1: Get distances
t_0 <- Sys.time()
...
t_f <- Sys.time()
print(t_f - t_0)
#> Time difference of 19.81907 secs

# Predict 2: Get speeds
t_0 <- Sys.time()
trip_speeds <- predict(object = traj,
                       new_times = trip_dists,
                       deriv = 1) %>%
  rename(speed = interp)
t_f <- Sys.time()
print(t_f - t_0)
#> Time difference of 18.60953 secs
```
...
stop_df <- trip_events %>%
  ...
  mutate(time_elapsed = round(time_elapsed, 1),
         lab = paste("*Stop ", row_number(), "*:<br>",
                     "**Time stopped**: ", time_elapsed, " s",
                     sep = ""),
...
sf_plot <- ggplot() +
  ...
  geom_segment(data = stop_df,
               aes(x = time_begin, xend = time_end, y = (y - 40), yend = (y - 40)),
               arrow = arrow(ends = "both", type = "closed", length = unit(0.025, units = "npc")),
               color = "firebrick", linetype = "solid", linewidth = 0.7) +
  geom_textbox(data = stop_df,
               aes(x = x, y = y, label = lab),
               hjust = 0.5, halign = 0.5, size = 2.5, color = "firebrick",
               alpha = 0.9, width = 0.22) +
  scale_linetype_manual(name = "Features",
                        values = c("Entrance/Exit" = "longdash",
                                   "Stopbar" = "dotted")) +
  ...
  labs(x = "Time (afternoon of 11/18/24)",
       y = "Distance from Beginning (m)",
       title = "Split Failure at Virginia & South/East",
       subtitle = paste("IndyGo Red Line 90 NB; Trip ID ", plot_trip,
                        sep = ""))
sf_plot
```

![](figures/sf_plot_lowres.png)
...
aog_lab <- aog %>%
  mutate(lab = paste("**AOG**: ", round(est_AOG * 100, 1), " %<br>n = ", num_trips, " trips",
                     sep = ""),
         y = est_AOG / 2)
sf_lab <- split_failures %>%
  mutate(lab = paste("**SF**: ", round(sf * 100, 2), " %<br>n = ", num_trips, " trips",
                     sep = ""),
         y = sf / 2)
...
aog_plot <- ggplot() +
  ...
  geom_textbox(data = aog_lab,
               aes(x = period, y = y, label = lab, color = period),
               alpha = 0.9, size = 2.8, halign = 0.5, hjust = 0.5, width = 0.37,
               show.legend = FALSE) +
  scale_fill_manual(values = c("Before (9/2024)" = "#f43155",
                               "After (11/2024)" = "#2f6ff8")) +
  scale_color_manual(values = c("Before (9/2024)" = "firebrick4",
...
![](figures/combo_plot_lowres.png)

## References

\[1\] M. M. Garcez Duarte and M. Sakr, “An experimental study of
existing tools for outlier detection and cleaning in trajectories,”
Geoinformatica, vol. 29, no. 1, pp. 31–51, Jan. 2025, doi:
10.1007/s10707-024-00522-y.

\[2\] R. K. Pearson, Y. Neuvo, J. Astola, and M. Gabbouj, “Generalized
Hampel Filters,” EURASIP J. Adv. Signal Process., vol. 2016, no. 1,
p. 87, Aug. 2016, doi: 10.1186/s13634-016-0383-6.

\[3\] J. Robbennolt, S. Munira, and S. D. Boyles, “A Comparative Study
of Spline-Based Trajectory Reconstruction Methods Across Varying
Automatic Vehicle Location Data Densities,” Aug. 28, 2025, arXiv:
arXiv:2509.00119. doi: 10.48550/arXiv.2509.00119.

\[4\] F. N. Fritsch and J. Butland, “A Method for Constructing Local
Monotone Piecewise Cubic Interpolants,” SIAM Journal on Scientific and
Statistical Computing, Jul. 2006, doi: 10.1137/0905021.

\[5\] Y. Huang, A. Abdelhalim, A. Stewart, J. Zhao, and H. Koutsopoulos,
“Reconstructing Transit Vehicle Trajectory Using High-Resolution GPS
Data,” 2023 IEEE 26th International Conference on Intelligent
Transportation Systems (ITSC), Bilbao, Spain, 2023, pp. 5247-5253.
<https://dspace.mit.edu/handle/1721.1/156441>

\[6\] Transportation Research Board and E. National Academies of
Sciences and Medicine, Performance-Based Management of Traffic Signals.
Washington, DC: The National Academies Press, 2020. doi: 10.17226/25875.

\[7\] J. Jackson, R. Huang, P. Koonce, and E. Burkman, “Use of
High-Resolution Signal Controller Data to Measure Transit Signal
Priority Performance: A Case Study in the Boston Region,” Transportation
Research Record, vol. 2678, no. 6, pp. 138–149, Jun. 2024, doi:
10.1177/03611981231194345.
