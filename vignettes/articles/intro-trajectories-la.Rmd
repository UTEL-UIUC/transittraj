...
In the previous vignette (`vignette("articles/data-workflow-la")`), we saw how we
...
You'll notice that, even though we have 11 trips, there were only two to four
...
beginning of `downtown_lims`, then `event_timestamp` increments 10 seconds per
...
``` {r, echo = FALSE}
library(vembedr)
embed_url("https://www.youtube.com/watch?v=ctW5dI1069o") %>%
  use_align("center")
```
...
plot above). Through the rest of dowtown, movements seem to be smoother.
...
``` {r, echo = FALSE}
embed_url("https://www.youtube.com/watch?v=F9Cg8bZNk5A") %>%
  use_align("center")
```
